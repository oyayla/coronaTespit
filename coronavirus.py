...
def sonucAt():
    ...
    if(i <= -2):
        sonuc.config(text="Nesin sen ?\n 2058 yılından gelen covid-19 aşılı bir zaman yolcusu mu ?", font = ("Times", 10, "italic bold"), bg = "green")
    if ((i > -2) & (i <= 2)):
        sonuc.config(text="Coronavirus bile bu kadar kararsız değil. \n Git bi doktora görün ne varmış öğren", font = ("Times", 10), bg = "orange")
    if(i > 2):
        sonuc.config(text="Yakında beyaz bir ışık görünecek.\n Sonra çok sıcak bir yere gideceksin haberin olsun :D", font = ("Arial Black", 10, "bold"), bg = "red")
